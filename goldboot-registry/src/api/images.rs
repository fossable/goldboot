...
use goldboot::registry::protocol::{
...
pub async fn manifest(
    ...
    let blob_bytes = tokio::task::spawn_blocking(move || -> Result<Vec<u8>> {
        ...
        if handle.directory.is_none() {
            handle
                .load(None)
                .map_err(|_| anyhow::anyhow!("encrypted images are not yet supported by the registry"))?;
        }
    ...
    Ok((
        [(header::CONTENT_TYPE, HeaderValue::from_static(MANIFEST_CONTENT_TYPE))],
        blob_bytes,
    )
        .into_response())
}
...
pub async fn clusters(
    ...
    let (range_start, range_end_inclusive) = parse_range_header(&headers, total_len).map_err(|_| StatusCode::RANGE_NOT_SATISFIABLE)?;

    let absolute_start = cluster_start + range_start;
    let absolute_len = range_end_inclusive - range_start + 1;

    let mut file = tokio::fs::File::open(&file_path).await.map_err(|_| StatusCode::NOT_FOUND)?;
...
fn parse_range_header(headers: &HeaderMap, total_len: u64) -> Result<(u64, u64)> {
    ...
    let (s, e) = body.split_once('-').ok_or_else(|| anyhow::anyhow!("malformed"))?;
...
pub async fn push(
    ...
    let result = tokio::task::spawn_blocking(move || -> Result<u64> {
        ...
        storage_clone.put(&name_c, &tag_c, std::io::Cursor::new(buf), None)
    })
    .await
    .map_err(|_| StatusCode::INTERNAL_SERVER_ERROR)?;
