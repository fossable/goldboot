...
use gtk4::prelude::*;
use gtk4 as gtk;
use gb_image::header_ext::HeaderExt;
...
impl ImageEntry {
    ...
    pub fn search() -> Vec<ImageEntry> {
        ...
        for search_path in vec!["."] {
            for read_result in std::fs::read_dir(search_path).unwrap()/*.filter(|entry| entry.unwrap().path().extension() == Some(".gb"))*/ {
            	if let Ok(image_path) = read_result {
            		if let Ok(image) = gb_image::open(image_path.path()) {
	                    for ext in image.header.extensions {
	                        match ext {
	                            HeaderExt::GoldbootMetadata{len, data} => {
	                                if let Ok(metadata) = serde_json::from_slice::<ImageMetadata>(&data) {
